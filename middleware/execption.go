	...
	return func(c *gin.Context) {
		defer func() {
			if err := recover(); err != nil {
				req := response.Response{c}
				DebugStack := ""
				for _, v := range strings.Split(string(debug.Stack()), "\n") {
					DebugStack += v + "<br>"
				}
				subject := fmt.Sprintf("【重要错误】%s 项目出错了！", config.App.Name)
				body := strings.ReplaceAll(MailTemplate, "{ErrorMsg}", fmt.Sprintf("%s", err))
				body  = strings.ReplaceAll(body, "{RequestTime}", time.Now().String())
				...
				m := gomail.NewMessage()
				//设置发件人
				m.SetHeader("From", config.Mail.SystemEmailUser)
				//设置发送给多个用户
				mailArrTo := strings.Split(config.App.ErrorNotifyEmail, ",")
				m.SetHeader("To", mailArrTo...)
				//设置邮件主题
				m.SetHeader("Subject",subject)
				//设置邮件正文
				m.SetBody("text/html", body)
				d := gomail.NewDialer(config.Mail.SystemEmailHost, config.Mail.SystemEmailPort, config.Mail.SystemEmailUser, config.Mail.SystemEmailPass)
				_ = d.DialAndSend(m)
				req.HttpFailed(http.StatusInternalServerError, "系统异常，请联系管理员！")
			}
		}()
		c.Next()
	}
