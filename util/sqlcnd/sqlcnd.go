...
func (s *SqlCnd) With(table string, conditions ...interface{}) *SqlCnd  {
	var preloads []With
	for _, preload := range s.preload {
		if preload.Column != table {
			preloads = append(preloads, preload)
		}
	}
	preloads = append(preloads, With{Column:table,Conditions:conditions})
	s.preload = preloads
	return s
}
...
func (s *SqlCnd) Build(db *gorm.DB) *gorm.DB {
	...
	// with
	if len(s.preload) > 0 {
		for _, with := range s.preload {
			if len(with.Conditions) > 0 {
				ret = ret.Preload(with.Column,with.Conditions...)
			} else {
				ret = ret.Preload(with.Column)
			}
		}
	}
	return ret
